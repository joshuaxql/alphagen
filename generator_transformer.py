...
class AlphaGenTransformer(nn.Module):
    ...
    def forward(
        ...
        hidden: Optional[Tuple[torch.Tensor, torch.Tensor]] = None  # 保持接口兼容，但Transformer不需要
    ...
    def forward_sequence(
        ...
        # 2. 创建填充掩码（标记哪些位置是填充的）
        padding_mask = torch.arange(max_len, device=lengths.device).expand(batch_size, max_len) >= lengths.unsqueeze(1)
    ...
    def init_hidden(self, batch_size: int = 1, device: Optional[torch.device] = None) -> None:
        """
